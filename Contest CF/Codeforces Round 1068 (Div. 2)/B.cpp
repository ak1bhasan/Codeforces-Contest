...
ll Tactical( vector<ll> &v1, vector<ll> &v2, int n )
{
    vector<ll> v3( n + 2, 0 ), v4( n + 2, 0 );

    v3[n + 1] = 0;
    v4[n + 1] = -INF;

    for( int i = n; i >= 1; i-- ) {
        v3[i] = max( -v1[i] + v3[i + 1], -v2[i] + v4[i + 1] );
        v4[i] = max( v1[i] + v4[i + 1], v2[i] + v3[i + 1] );
    }
    return max( v3[1], v4[1] );
